...
An issue with sampling \textit{idle} stalls is that the \acrshortpl{sm} terminate at different times and thus also dump their performance data at different times. The \acrshortpl{sm} will therefore not track idle cycles after completing, while other \acrshortpl{sm} continue execution. The idle cycles can thus not be read by the \acrshortpl{sm} themselves. To solve this, the number of idle cycles could be calculated as
